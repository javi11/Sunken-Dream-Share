package game.levels {
	...
	/**
	 * Cuerda para colgarse y balancearse..
	 */
	public class Rope extends Box2DPhysicsObject {
		
		/**
		 * El objeto donde esta enganchado la cuerda  
		 */
		public var anchor:Box2DPhysicsObject;		
		public var ropeLength:uint = 120;
		...
		/**
		 * Textura de los segmentos 
		 */
		public var segmentBitmapData:BitmapData;
		/**
		 * La posicion donde esta enganchado el player
		 */
		public var heroAnchorOffset:b2Vec2;
		/**
		 * El impulso aplicado cuando el player coje la cuerda
		 */
		...
		override protected function defineFixture():void {
			...
			for (var i:uint = 0; i < numSegments; ++i) 
			{
				...
				fixtureDefRope.density = 4;
				fixtureDefRope.friction = 0.75;
		...
		override public function handlePreSolve(contact:b2Contact, oldManifold:b2Manifold):void {
			...
			if (Box2DUtils.CollisionGetOther(this, contact) is Hero){
				if (!ropeAdded  && !hero.body.GetJointList()) 
				{
					...
					onHang.dispatch();
					(hero as Player).isHanging = true; 
					(hero as Player).currentRope = this.name;
				}
			}
		}
		...
		public function removeJoint():void
		{
			...
			(hero as Player).isHanging = false;
			onHangEnd.dispatch();
